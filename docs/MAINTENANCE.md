...
生成的统计图由本仓库托管，README 主要使用相对路径；仓库概览和 Star 卡使用带一次性版本参数的 raw 链接，刷新旧 Metrics 图片缓存。版本参数不是实时更新机制。关注者卡使用新的 `people.svg` 路径，避免旧头像图片缓存。更改 SVG 后 GitHub 图片缓存可能需要一点时间刷新。生成失败时保留上次已提交的图片，需查看相应 Actions 日志确认更新时间。
