...
- Metrics 账号概览、关注者、语言分析、评论反应、Issues/PR 跟进、最近 Star、Star 趋势，以及精选公开项目 `minigo` 和 `tiktok`。精选卡不改变 GitHub 原生置顶。
- 折叠区域中的公开推送时段、最近活动与 Discussions 统计。
...
GitHub 模块仅使用仓库自动提供的临时 `GITHUB_TOKEN`，无需新增 PAT。关闭 `plugin_gists` 是因为临时令牌不能读取用户 Gists；开启会造成 Metrics 渲染失败。`only_base=true` 可单独重生成账号概览。

近期活动和推送时段由 `scripts/public_activity.py` 读取公开 Events API 生成，避免上游 Metrics 旧插件依赖已移除的 `payload.commits` 字段。

Metrics 设置 `repositories_affiliations: ''`，避免上游把仓库令牌误当用户身份而只统计当前主页仓库。Metrics 的仓库范围包括相关的公开协作仓库，且按默认配置排除 fork；与 GitHub 主页上的全部自有公开仓库数量口径不同。
